...
const token_class_1 = __importDefault(require("../classes/token.class"));
...
usuarioRutas.post('/crear', (req, res) => {
    ...
    //gurdar datos en bd
    usuario_model_1.Usuario.create(usuario)
        .then(usuarioDB => {
        res.json({
            ok: true,
            usuario: usuarioDB
...
usuarioRutas.post('/entrar', (req, res) => {
    const body = req.body;
    usuario_model_1.Usuario.findOne({ nombre: body.nombre }, (err, usuarioBD) => {
        if (err)
            throw err;
        if (!usuarioBD) {
            return res.json({
        ...
        if (usuarioBD.compararContrasena(body.password)) {
            ...
            const miToken = token_class_1.default.getToken({
                _id: usuarioBD._id,
...
usuarioRutas.post('/update', autentificacion_middleware_1.verificarToken, (req, res) => {
    const usuario = {
        nombre: req.body.nombre || req.usuario.nombre,
        password: req.body.password || req.usuario.password
    };
    usuario_model_1.Usuario.findByIdAndUpdate(req.usuario._id, usuario, { new: true }, (err, userDB) => {
        if (err)
            throw err;
        if (!userDB) {
            return res.json({
